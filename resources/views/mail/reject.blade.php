<table align="center" bgcolor="#f9fafc" border="0" cellpadding="0" cellspacing="0" style="font-family:Helvetica,sans-serif;width:100%;max-width:598px;padding:0 20px">
    ...
    <tr>
        <td style="padding:0 20px 40px;color:#4a4a4a;background-color:#fff;border:solid 1px #dfebf9;font-size:14px;line-height:1.36">
            <img alt="" src="{{ url('mail_logo.png') }}" style="border:0;display:block;margin:0 auto;margin-bottom:30px;width:98px;height:40px;padding-top:28px" tabindex="0">
            <p>Sizin {{ $id }} - <a href="{{ route('auto.view', $id) }}" style="color:#2484c5;text-decoration:none" target="_blank">{{ $car->car_brand['name'] }} {{ $car->car_model['name'] }}</a> elanınız moderator tərəfindən dərc edilmədi.</p>
        </td>
    </tr>
    <tr>
        <td>
            <table align="right" border="0" cellpadding="0" cellspacing="0"style="font-family:Helvetica,sans-serif;padding:0">
                ...
                <tr>
                    <td>
                        <p style="margin:0;font-size:13px;font-weight:300;color:#4a4a4a;text-align:right">© {{ date('Y') }} <a href="http://TURBO.AZ" target="_blank">{{ strtoupper(get_setting('site_name')) }}</a></p>
                    </td>
                </tr>
                </tbody>
            </table>
