        ...
        <div class="section-title-container">
            <div class="section-title">
                <ul class="breadcrumbs">
                    ...
                    <li class="breadcrumbs-i"><a href="{{ route('autosalon.view', $car->company_id) }}">{{ $car->company['name'] }}</a></li>
                    <li class="breadcrumbs-i"><a href="{{ route('autos') }}?car_brand_id={{ $car->car_brand_id }}">{{ $car->car_brand['name'] }}</a></li>
                    <li class="breadcrumbs-i"><a href="{{ route('autos') }}?car_brand_id={{ $car->car_brand_id }}&car_model_id={{ $car->car_brand_id }}">{{ $car->car_model['name'] }}</a></li>
                </ul>
            </div>
        </div>
            ...
            <div class="shop-page-head">
                <div class="shop-header">
                    <div class="shop-header-left">
                        <div class="shop--logo" style="background-image: url({{ url('uploads/'.$car->company['image']) }})"></div>
                        <a class="shop--products-count" href="/avtosalonlar/mercedes-benz-absheron">{{ $car->company['autos']->count() }} {{ __('front.elan') }}</a>
                    </div>
                    <div class="shop-header-right">
                        <div class="shop-header-text">
                            <h1 class="shop--title"><a href="{{ route('autosalon.view', $car->company_id) }}">{{ $car->company['name'] }}</a></h1>
                            <div class="shop-header-table">
                                <div class="shop-header-cell shop-header-cell_description">
                                    <h2 class="shop--description">{{ $car->company['description'] }}</h2>
                                    <div class="shop--location-wrap">
                                        <a data-reveal-id="shop-show-map" class="shop--location shop--open-location-modal" href="#">
                                            <i class="fas fa-map-marker-alt"></i>{{ $car->company['address'] }}
                                        </a>
                                    </div>
                                    <div class="reveal-modal reveal-modal--map" id="shop-show-map">
                                        <div class="reveal-modal-popup">
                                            <div class="reveal-content">
                                                <div data-lat="{{ json_decode($car->company['location'])->location_lat }}" data-lng="{{ json_decode($car->company['location'])->location_lng }}" id="map"></div>
                                ...
                                <div class="shop-header-cell shop-header-cell_feedback">
                                    <div class="shop-phones">
                                        ...
                                        <div class="shop-phones-content">
                                            <div class="shop-phones-i"><a class="shop-phones--number" data-register-call="true" href="tel:(050) 289-09-85">{{ $car->company['phone'] }}</a></div>
                                        </div>
                                    </div>
                                    <div class="shop--schedule">
                                        <i class="far fa-clock"></i>
                                        <div class="shop--schedule-i">{{ $car->company['working_days'] }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="shop--special-offer"><i class="far fa-star"></i>{{ $car->company['slogan'] }}</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="product-container">
                <div class="product credit">
                    <div class="product-photos">
                        <a class="product-photos-large" href="{{ $car->product_photos_large() }}">
                            <img alt="{{ $car->car_brand['name'] }} {{ $car->car_model['name'] }}" src="{{ $car->product_photos_large() }}" />
                        </a>
                        <div class="product-photos-thumbnails product-photos-thumbnails_l">
                            @if($car->images)
                                @foreach($car->images as $image)
                                    <a href="{{ asset('uploads/'.$image->image) }}">
                                        <img alt="{{ $car->car_brand['name'] }} {{ $car->car_model['name'] }}" src="{{ asset('uploads/'.$image->image) }}" />
                                    </a>
                                @endforeach
                            @endif
                        </div>
                        ...
                        <div class="services">
                            <a id="bump_open" class="services-i services-i_bump" data-reveal-id="bump_modal" data-animation="fade" href="#bump_open">{{ __('front.Pull the ad forward') }}
                                <span class="services-i-price">
                                    ...
                                    <span class="price-cur">3 {{ $car->currency['code'] }}</span>
                                </span>
                            </a>
                            <a id="vip_open" class="services-i services-i_vip" data-reveal-id="vip_modal" data-animation="fade" href="#vip_open">{{ __('front.make the ad VIP') }}
                                <span class="services-i-price">
                                <span class="price-val">{{ __('front.from') }}</span>
                                <span class="price-cur">15 {{ $car->currency['code'] }}</span>
                                </span>
                            </a>
                            <a id="featured_open" class="services-i services-i_featured" data-reveal-id="featured_modal" data-animation="fade" href="#featured_open">{{ __('front.make the ad premium') }}<span class="services-i-price"><span class="price-val">{{ __('front.from') }}</span><span class="price-cur">5 {{ $car->currency['code'] }}</span></span></a>
                        </div>
                    </div>
            ...
            <div class="w-100">
                <div class="section-title-container products">
                    ...
                    <div class="section-title">
                        ...
                        <a class="section-title_more" target="_blank" href="{{ route('autosalon.view', $car->company_id) }}">{{ __('front.show them all') }}</a>
                    </div>
                </div>
