...
@section('title', __('company.company')." ".__('messages.group'))
@section('content')
    <div class="page-content pt-0">
        <div class="content-wrapper">
            <div class="content">
                ...
                <div class="card">
                    <div class="card-body">
                        <x-create route="admin.company_group.create"/>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            @foreach($company_groups as $company_group)
                                <tr>
                                    <td>{{ $company_group->id }}</td>
                                    <td>{{ $company_group->name }}</td>
                                    <td>
                                        {!! $company_group->status ? '<span class="badge badge-success">'.__('messages.enable').'</span>' : '<span class="badge badge-danger">'.__('messages.disable').'</span>' !!}
                                    </td>
                                    <td>{{ $company_group->created_at->diffForHumans() }}</td>
                                    <td>{{ $company_group->updated_at->diffForHumans() }}</td>
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <x-edit route="admin.company_group.edit" :id="$company_group->id"/>
                                            <x-delete route="admin.company_group.destroy" :id="$company_group->id"/>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
