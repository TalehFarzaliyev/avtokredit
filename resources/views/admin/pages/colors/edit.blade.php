...
@section('title', __('color.title_edit'))
@section('content')
    <!-- Page content -->
                                    ...
                                    <div class="tab-pane fade show {{$loop->index == '0' ? 'active' : ''}}" id="card-{{$key}}">
                                        <div class="form-row">
                                            <div class="form-group col-sm-6">
                                                ...
                                                <input type="text" class="form-control " name="{{'name:'.$key}}" id="name-{{ $key }}"  value="{{ old('name:'.$key, $color->{'name:'.$key}) }}">
                                                @error('name:'.$key)
                                ...
                                <div class="form-row">
                                    <div class="form-group col-sm-12">
                                        ...
                                        <select name="status" class="select">
                                            <option {{old('status', $color->status) == 1 ? 'selected': ''}} value="1">{{__('messages.enable')}}</option>
                                            <option {{old('status', $color->status) == '0' ? 'selected': ''}} value="0">{{__('messages.disable')}}</option>
                                        </select>
                                        @error('status')
                                        <span class="form-text font-weight-semibold text-danger">{{ $message }}</span>
                                        @enderror
                                    ...
                                    <div class="col-sm-12">
                                        <x-save />
                                        <x-back route="admin.color.index"></x-back>
                                    </div>
                                </div>
                            </div>
                        </div>
